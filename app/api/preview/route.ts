...
/**
 * GET /api/preview?syncId=<syncId>
 * Fetches the file content from GitHub for preview (first 2000 characters)
 */
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // Return first 2000 characters for preview
    const previewContent = fullContent.slice(0, 2000);
    const isTruncated = fullContent.length > 2000;

    return NextResponse.json({
      content: previewContent,
      isTruncated,
      fullLength: fullContent.length,
      fileName: historyEntry.filePath.split("/").pop() || "document.md",
    });
  } catch (error) {
